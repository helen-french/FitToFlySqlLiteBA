...
export default function HistoryScreen() {
  ...
  // System Time Anchor locked to your active data window
  ...
  const loadHistoryLogs = useCallback(async () => {
    try {
      ...
      const startOfMonthISO = new Date(year, month, 1, 0, 0, 0).toISOString();
      ...
      // Core Query: Ordered by desc() to ensure latest adjustments appear first
      const baseAmendments = await db
        .select()
        .from(rosterAmendments)
        .where(
          and(
            gte(rosterAmendments.createdAt, startOfMonthISO),
            lte(rosterAmendments.createdAt, endOfMonthISO),
          ),
        )
  ...
  const renderHistoryItem = ({ item }: { item: HydratedHistoryRow }) => {
    const isExpanded = !!expandedRows[item.id];

    // Switch Matrix: Chronological chronological segregation for explicit future layout safety
    switch (item.amendment.itemType) {
      case "T": // Variant 1: Trip Assignment Change (Expandable pipeline layout)
        return (
          <View
            style={[
              styles.historyCard,
      ...
      case "G": // Variant 2: Ground Duty Changes Fallback shell
        return (
          <View
            style={[
              styles.historyCard,
      ...
      case "D": // Variant 3: Micro Duty Adjustments (Check-in variations)
      case "S": // Variant 4: Specific Single Sector Swap notifications
      default:
        return (
          <View
            style={[
